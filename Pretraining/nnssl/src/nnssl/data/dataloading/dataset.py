...
class nnSSLDatasetBlosc2(nnSSLBaseDataset):
    # Cache for grouping modalities per subject/session to avoid repeated scans.
    _multi_modal_cache: dict[int, dict[tuple[str, str, str, str], list[IndependentImage]]] = {}
    ...
    def __getitem__(self, image_identifier):
                try:
                    data, anon, anat, properties = self.load_case(self.dataset_dir, self.image_dataset, image_identifier)
                except RuntimeError:
                    # fallback to a random sample if loading fails
                    return self.__getitem__(choice(self.image_identifiers))

                # Get the true filename / identifier
                # This is the unique ID, e.g. ABC_0001_GH_010208
                filename = image_identifier  
                # print(filename)

                # Wrap into dict for the dataloader
                return {
                    "data": np.asarray(data, dtype=np.float32),
                    "anon": None if anon is None else np.asarray(anon, dtype=np.uint8),
                    "anat": None if anat is None else np.asarray(anat, dtype=np.uint8),
    ...
    def load_case(dataset_dir: str, image_dataset: dict[str, IndependentImage], image_identifier: str):
        ...
        subject_key = (
            str(img.collection_name),
            str(img.dataset_index),
            str(img.subject_id),
            str(img.session_id),
        )

        dataset_cache = nnSSLDatasetBlosc2._multi_modal_cache.setdefault(id(image_dataset), {})
        if subject_key not in dataset_cache:
            siblings = [
                other
                for other in image_dataset.values()
                if other.subject_id == img.subject_id
                and other.session_id == img.session_id
                and other.dataset_index == img.dataset_index
                and other.collection_name == img.collection_name
            ]
            dataset_cache[subject_key] = siblings
        else:
            siblings = dataset_cache[subject_key]

        modality_priority = ("T2W", "ADC", "HBV")

        def sort_key(iimg: IndependentImage):
            if iimg.image_modality in modality_priority:
                return (0, modality_priority.index(iimg.image_modality))
            return (1, iimg.image_modality or "")

        sorted_siblings = sorted(siblings, key=sort_key)
        unique_modality_map: dict[str, IndependentImage] = {}
        for sib in sorted_siblings:
            mod = sib.image_modality
            if mod in modality_priority and mod not in unique_modality_map:
                unique_modality_map[mod] = sib
        selected_siblings = [unique_modality_map[mod] for mod in modality_priority if mod in unique_modality_map]

        if len(selected_siblings) > 1:
            modal_order = []
            raw_channels: list[np.ndarray] = []
            for sibling in selected_siblings:
                modal_order.append(sibling.image_modality)
                sibling_img_path = sibling.get_output_path("image", ext=".b2nd")
                sibling_handle = blosc2.open(
                    urlpath=join(dataset_dir, sibling_img_path),
                    mode="r",
                    dparams=dparams,
                    mmap_mode="r",
                )
                sibling_np = np.asarray(sibling_handle)
                if sibling_np.ndim == 4 and sibling_np.shape[0] == 1:
                    sibling_np = sibling_np[0]
                raw_channels.append(np.asarray(sibling_np, dtype=np.float32))
                if hasattr(sibling_handle, "close"):
                    sibling_handle.close()

            if raw_channels:
                target_shape = tuple(np.max(np.array([arr.shape for arr in raw_channels]), axis=0))

                def align_shape(arr: np.ndarray, target: tuple[int, int, int]) -> np.ndarray:
                    result = arr
                    for axis, desired in enumerate(target):
                        size = result.shape[axis]
                        if size > desired:
                            start = (size - desired) // 2
                            end = start + desired
                            result = result.take(indices=range(start, end), axis=axis)
                    pad_width = []
                    for axis, desired in enumerate(target):
                        size = result.shape[axis]
                        if size < desired:
                ...
                data_channels = [align_shape(channel, target_shape) for channel in raw_channels]
                data = np.stack(data_channels, axis=0)

                base_img = selected_siblings[0]
                base_properties_path = join(dataset_dir, base_img.get_output_path("image", ext=".pkl"))
                properties = load_pickle(base_properties_path)
                properties["modalities"] = modal_order

                anon_mask_path = join(dataset_dir, base_img.get_output_path("anon_mask", ext=".b2nd"))
                if isfile(anon_mask_path):
                    anon_handle = blosc2.open(urlpath=anon_mask_path, mode="r", dparams=dparams, mmap_mode="r")
                    anon = np.asarray(anon_handle)
                    if hasattr(anon_handle, "close"):
                        anon_handle.close()
                else:
                    anon = None

                anat_mask_path = join(dataset_dir, base_img.get_output_path("anat_mask", ext=".b2nd"))
                if isfile(anat_mask_path):
                    anat_handle = blosc2.open(urlpath=anat_mask_path, mode="r", dparams=dparams, mmap_mode="r")
                    anat = np.asarray(anat_handle)
                    if hasattr(anat_handle, "close"):
                        anat_handle.close()
                else:
                    anat = None
