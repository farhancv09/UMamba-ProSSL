...
class VolumeFusionTransform(AbstractTransform):
    ...
    def __call__(self, **data_dict):
        ...
        result = {"input": mixed_images, "target": masks}

        filenames = data_dict.get("filenames")
        properties = data_dict.get("properties")
        keys = data_dict.get("keys")

        if filenames is not None or properties is not None or keys is not None:
            half_batch = mixed_images.shape[0]
            metadata = []
            for idx in range(half_batch):
                fg_idx = idx
                bg_idx = idx + half_batch

                entry = {}
                if filenames is not None:
                    entry["image_file"] = filenames[fg_idx] if fg_idx < len(filenames) else None
                    entry["mix_image_file"] = filenames[bg_idx] if bg_idx < len(filenames) else None
                    entry["basename"] = os.path.basename(entry["image_file"]) if entry["image_file"] else None
                if properties is not None:
                    entry["properties"] = {
                        "foreground": properties[fg_idx] if fg_idx < len(properties) else None,
                        "background": properties[bg_idx] if bg_idx < len(properties) else None,
                    }
                if keys is not None:
                    entry["keys"] = {
                        "foreground": keys[fg_idx] if fg_idx < len(keys) else None,
                        "background": keys[bg_idx] if bg_idx < len(keys) else None,
                    }
