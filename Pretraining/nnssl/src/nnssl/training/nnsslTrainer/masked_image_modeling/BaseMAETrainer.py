...
class BaseMAETrainer(AbstractBaseTrainer):
    ...
    def __init__(
        ...
        self.output_folder = "/mnt/work1/farhan/Farhan/SSL_study/ssl_figures/mae"
        if self.save_batch:
    ...
    def train_step(self, batch: dict) -> dict:
        ...
        data = data.permute(0, 1, 3, 4, 2).contiguous()
        masked_data = masked_data.permute(0, 1, 3, 4, 2).contiguous()
        mask  = mask.permute(0, 1, 3, 4, 2).contiguous()
        # breakpoint()
...
class BaseMAETrainerUMambaBot(BaseMAETrainer):
    """
    MAE pre-training using a custom-configured UMambaBot encoder/decoder.
    """
    ...
    def build_architecture_and_adaptation_plan(
        ...
        # For MAE we reconstruct the full input volume, so enforce output channels to match input channels.
